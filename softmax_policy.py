# From Mark Ho's MSDM Codebase; use until he changes the SoftMaxDistribution to DictDistribution in from_q_matrix function
...
class TabularPolicy(dict, Policy):
    ...
    def from_matrix(cls, states, actions, policy_matrix: np.array):
        ...
        for si, s in enumerate(states):
            adist = {}
            assert math.isclose(policy_matrix[si, :].sum(
            ), 1), f"Sums to {policy_matrix[si, :].sum()}"
    ...
    def from_q_matrix(cls, states, actions, q: np.array, inverse_temperature=float('inf')):
        ...
        if inverse_temperature == float('inf'):
            ...
            policy_ismax = \
                np.isclose(q, np.max(q, axis=-1, keepdims=True),
                           rtol=0.0, atol=atol)
            ...
            for si, s in enumerate(states):
                ...
                if policy_counts[si] == 1:
                    policy[s] = DeterministicDistribution(
                        actions[binary_policy.argmax()])
                else:
                    policy[s] = UniformDistribution(
                        [actions[ai] for ai in np.where(binary_policy)[0]])
        else:
            pi = np.exp(inverse_temperature *
                        (q - np.max(q, axis=-1, keepdims=True)))
            pi = pi/pi.sum(axis=-1, keepdims=True)
            policy = {}
            for si, s in enumerate(states):
                policy[s] = DictDistribution(dict(zip(actions, pi[si])))
        return TabularPolicy(policy)
